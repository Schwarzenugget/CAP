...
const int NUMRPAT = 17;		//Patrons per reconèixer
...
double *WeightHO[NUMOUT];	//Pesos entre neurones ocultes i de


/*AllWeights contindrà tots els Weights de cada process, és un buffer*/
double **AllWeightsIH;	//Pesos entre neurones d'entrada i ocultes
double **AllWeightsHO;	//Pesos entre neurones ocultes i de sortida
...
double rando()
{
	return ((double)(rand()/((RAND_MAX+1)*1.0)));
}
